...
class WC_Gateway_DNotesPay extends WC_Payment_Gateway {
	...
	public function is_available() {
	    if ( $this->enabled && $this->wallet && $this->dnotespay_is_valid_currency() && $this->dnotespay_check_hold_stock() === true && $this->dnotespay_is_time_limit_correct() === true ) {
	        return true;
	    } else {
	        return false;
	    }
	...
	public function dnotespay_get_confirm_num( ) {
		...
		if ( $valid_confirm_num !== true ) {
			$confirm_num = $valid_confirm_num;
		} else {
	...
	public function dnotespay_is_time_limit_correct() {
		if ( $this->payment_waiting_time > 0 && $this->payment_processing_time > 0 && $this->payment_processing_time > $this->payment_waiting_time ) {
			return true;
		} else {
			return false;
		}
	...
	public function dnotespay_convert_order_total( $order_total ) {
	    ...
	    $send_amount = round(( $order_total / $usd_price ) , 5);
